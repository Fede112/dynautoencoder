...
def estimate(X,fraction=0.9,verbose=True):    
    ...
    good = np.setdiff1d(np.arange(X.shape[0]), np.array(zeros) )
    good = np.setdiff1d(good,np.array(degeneracies))
    
    if verbose:
        print('Fraction good points: {}'.format(good.shape[0]/X.shape[0]))
...
def block_analysis(X, blocks=list(range(1, 21)), fraction=0.9):
    ...
    for b in tqdm(blocks):
        ...
        for i in range(b):
            if (b == 1):
                _,_,reg,_,_ = estimate(X,fraction=fraction,verbose=False)
            else:    
                I = np.meshgrid(split[i], split[i], indexing='ij')
                tX = X[tuple(I)]
                # tX = X[split[i],:][:,split[i]]
